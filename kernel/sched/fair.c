...
enum AppType{CPU_APP, GPU_APP, UNKNOWN_APP};
enum KernelType{Computer_Intensive, Memory_Intensive, UNKNOWN_Type};
#define Queue_Size_Increment 10
struct GPU_sched_class {
	bool IsOpened;
	KernelType* Running_Queue;
	unsigned int* Kernel_Index;
	unsigned int Queue_Space;
	unsigned int Compute_Load;
	unsigned int Memory_Load;
	unsigned int On_Queue_Task_Number;
	unsigned int Total_Load;
	KernelType Intensive;

};
struct GPU_sched_class GPU_sched = {false, NULL, NULL, 0, 0,0,0,UNKNOWN_Type};
bool start_GPU_sched()
{  
	if (GPU_sched.IsOpened == true)
	{
		return false;
	}
	GPU_sched.Running_Queue = (KernelType*)malloc(Queue_Size_Increment * sizeof(KernelType));
	if (GPU_sched.Running_Queue == NULL)
	{
		return false;
	}
	GPU_sched.Kernel_Index = (unsigned int*)malloc(Queue_Size_Increment * sizeof(unsigned int));
	if (GPU_sched.Kernel_Index == NULL)
	{
		free(GPU_sched.Running_Queue);
		GPU_sched.Running_Queue = NULL;
		return false;
	}
	Queue_Space += Queue_Size_Increment;
	GPU_sched.IsOpened = true;
	return true;
}
bool Insert_GPU_Task(struct task_struct* p)
{
	if (p->IsGPUKernel == false)
	{
		return false;
	}
	if (GPU_sched.IsOpened == false)
	{
		return false;
	}
	if (GPU_sched.On_Queue_Task_Number  ==  GPU_sched.Queue_Space)
	{
		new_Runq = (KernelType*)malloc((GPU_sched.Queue_Space+ Queue_Size_Increment) * sizeof(KernelType));
		new_Indexq = (unsigned int*)malloc((GPU_sched.Queue_Space + Queue_Size_Increment) * sizeof(unsigned int));
		if ((new_Runq == NULL) || (new_Indexq == NULL))
		{
			if (new_Runq != NULL)
			{
				free(new_Runq);
			...
			for (unsigned int i = 0; i <= GPU_sched.Queue_Space - 1; i++)
			{
				new_Runq[i] = GPU_sched.Running_Queue[i];
				new_Indexq[i] = GPU_sched.Kernel_Index[i];
			}
			GPU_sched.Running_Queue = new_Runq;
			GPU_sched.Kernel_Index = new_Indexq;
			GPU_sched.Queue_Space += Queue_Size_Increment;
		}
	}
	GPU_sched.On_Queue_Task_Number ++;
	GPU_sched.Total_Load++;
	GPU_sched.Kernel_Index[GPU_sched.Total_Load - 1] = GPU_sched.Total_Load;
	p->Kernel_Index = GPU_sched.Total_Load;
	If(p->C_Mmode_queue[p->exe_position] == true)
	{
		GPU_sched.Compute_Load++;
		GPU_sched.Running_Queue[GPU_sched.On_Queue_Task_Number - 1] = Computer_Intensive;
	}
	else
	{
		GPU_sched.Memory_Load++;
		GPU_sched.Running_Queue[GPU_sched.On_Queue_Task_Number - 1] = Memory_Intensive;
	}
	if (GPU_sched.Memory_Load > GPU_sched.Compute_Load)
	{
		GPU_sched.Intensive = Memory_Intensive;
	}
	else if (GPU_sched.Memory_Load < GPU_sched.Compute_Load)
	{
		GPU_sched.Intensive = Computer_Intensive;
	}
	else
	{
		GPU_sched.Intensive = UNKNOWN_Type;
	}
	return true;
}
bool Delete_GPU_Task(struct task_struct* p)
{
	if (p->IsGPUKernel == false)
	{
		return false;
	...
	unsigned int flag = p->Kernel_Index;
	unsigned int Aim_Index;
	for (unsigned int i = 0; i <= GPU_sched.On_Queue_Task_Number - 1; i++)
	{
		if (GPU_sched.Kernel_Index[i] == flag)
		{
			Aim_Index = i;
		}
		break;
	}
	for (unsigned int i = Aim_Index; i <= GPU_sched.On_Queue_Task_Number - 2; i++)
	{
		GPU_sched.Kernel_Index[i] = GPU_sched.Kernel_Index[i + 1];
		GPU_sched.Running_Queue[i] = GPU_sched.Kernel_Index[i + 1];
	}
	GPU_sched.On_Queue_Task_Number--;
	if (p->C_Mmode_queue[p->exe_position] == true)
	{
...
bool end_GPU_sched()
{
	...
	free(GPU_sched.Running_Queue);
	free(GPU_sched.Kernel_Index);
	GPU_sched.Compute_Load = 0;
	GPU_sched.On_Queue_Task_Number = 0;
	GPU_sched.Memory_Load = 0;
	GPU_sched.Queue_Space = 0;
	GPU_sched.Total_Load = 0;
	GPU_sched.Running_Queue = NULL;
	GPU_sched.Kernel_Index = NULL;
	GPU_sched.Intensive = UNKNOWN_Type;
	return true;
}
 ...
 */
bool Is_Intensive_Proper(struct sched_entity* se)
{
	...
	bool flag = p->C_Mmode_queue[p->exe_position];
	if (flag == true && GPU_sched.Intensive == Computer_Intensive)
	{
		return true;
	}
	else if(flag == false && GPU_sched.Intensive == Memory_Intensive)
	{
		return true;
	}
	return false;
...
pick_next_entity(struct cfs_rq *cfs_rq, struct sched_entity *curr)
{
	...
	{
		if ((temp != NULL) && (!Is_Intensive_Proper(temp) ))
		{
